...
# ==============================================================================
CONDITIONS = [
    {"name": "Q = 0.01 mL/min, Pin = 1 mM", "col_t": 9,  "col_p": 10, "id": "cond1"},
    {"name": "Q = 0.03 mL/min, Pin = 1 mM", "col_t": 11, "col_p": 12, "id": "cond2"},
    {"name": "Q = 0.04 mL/min, Pin = 1 mM", "col_t": 13, "col_p": 14, "id": "cond3"}
]
...
# ==============================================================================
# 2. CFSTR BASE PARAMETERS & NUCLEATION CONSTANTS (Thesis Section 5.2)
# ==============================================================================
BASE_CONFIG = {
    ...
    "MW_cal": 100.0869,      # CaCO3 molecular weight (g/mol)
    "Ksp_cal": 10**(-8.48),
    
    # Hydroxyapatite (HA) Physical Properties (Thesis Chapter 5, Table 5.3)
    "MW_HA": 502.31,         # g/mol
    "rho_HA": 3100.0,        # kg/m3
    "sigma_HA": 0.087,       # Surface free energy (J/m2)
    "a_HA": 0.8,             # Aspect ratio width
    "b_HA": 0.047,           # Aspect ratio thickness
    "Ksp_HA": 10**(-44.333), # Equilibrium solubility constant with H+
    "Np_HA": 8.62e6,         # Total nucleated particles (Section 5.3.9.1)
    
    # Speciation & Activities at pH ~ 7.5
    "gamma_Ca": 0.87,
    "gamma_CO3": 0.90,
    "gamma_PO4": 0.73,
    "alpha0": 0.0967,
    "kgl": 10**(-2.145),     # min^-1
    "CO3_sat": 10**(-5.0),
    "H_act": 10**(-7.50)     # {H+} at pH 7.5
}

# ==============================================================================
# 3. EXPLICIT EULER FORWARD CFSTR INTEGRATOR (Thesis Appendix 5D)
# ==============================================================================
def simulate_cp_euler(params, config, t_max):
    """
    Solves CFSTR mass balances using Euler Forward method (h = 1.0 min)
    matching thesis Appendix 5D and Excel model.
    """
    dt = 1.0  # min
    ...
    # Particle geometry factors
    v_HA = config["MW_HA"] / (config["rho_HA"] * 1000.0)  # m3/mol
    geom_factor = 1.0 + (1.0 / config["a_HA"]) + (1.0 / config["b_HA"])
    ...
    for step in range(n_steps):
        ...
        P_uM = P * 1e6
        Ps_star = 0.084 * (max(0.0, P_uM)**0.31)
        ...
        # 3. Hydroxyapatite Nucleation & Precipitation
        # At pH 7.5, free PO4^3- fraction ~ 9.4e-6
        PO4_act = config["gamma_PO4"] * P * 9.4e-6
        Ca_act = config["gamma_Ca"] * Ca
        
        # CORRECTED IAP FORMULA: H+ in denominator
        IAP_HA = (Ca_act**5) * (PO4_act**3) / config["H_act"]
        
        # Saturation state Omega = (IAP / Ksp)^(1/9)
        if IAP_HA > 0:
            Omega_HA = (IAP_HA / config["Ksp_HA"])**(1.0 / 9.0)
        else:
            Omega_HA = 0.0
        ...
        if (not nucleated) and (Omega_HA >= Omega_star_HA):
            ...
            Lc = geom_factor * (v_HA * config["sigma_HA"]) / (8.314 * 298.15 * ln_om)
            L_HA = Lc
            CHA = 1000.0 * config["Np_HA"] * config["rho_HA"] * config["a_HA"] * config["b_HA"] * (L_HA**3) / V
            
        # Crystal Growth
        if nucleated:
            SSA_HA = (2.0 * (config["a_HA"] * config["b_HA"] + config["a_HA"] + config["b_HA"])) / (
                1000.0 * config["rho_HA"] * config["a_HA"] * config["b_HA"] * max(1e-9, L_HA)
            )
            r_precip_HA = kHA * SSA_HA * CHA * f_G_HA * 1e-6  # mol/(L*min)
            
            # Growth along length L
            dL_dt = (r_precip_HA * config["MW_HA"] * V) / (
                3000.0 * config["Np_HA"] * config["rho_HA"] * config["a_HA"] * config["b_HA"] * max(1e-9, L_HA**2)
            )
            L_HA += dL_dt * dt
            CHA = 1000.0 * config["Np_HA"] * config["rho_HA"] * config["a_HA"] * config["b_HA"] * (L_HA**3) / V
        else:
        ...
        dP_dt = (Q / V) * (Pin - P) - Ccal * dPs_dt * 1e-6 - 3.0 * r_precip_HA
...
def run_forward_model(params, time_points, config):
    ...
    # Interpolate to experimental sampling times
...
def log_prior(theta):
    ...
    if not (-7.5 <= log10_kP <= -3.0):
        return -np.inf
    if not (-18.0 <= log10_kHA <= -12.0):
        return -np.inf
    if not (1.01 <= Omega_star_HA <= 1.40):
        return -np.inf
    if not (-7.0 <= log_sigma_P <= -3.0):
        return -np.inf
...
def log_likelihood(theta, exp_data, config):
    ...
    params = {
        ...
        "log10_kcal": -5.90  # Invariant calcite dissolution
    }
    
    P_sim = run_forward_model(params, exp_data["t_min"], config)
    res = exp_data["P_meas"] - P_sim
    n = len(exp_data["t_min"])
    return -0.5 * np.sum((res / sigma_P)**2) - n * np.log(sigma_P * np.sqrt(2 * np.pi))
...
if __name__ == "__main__":
    ...
    for idx, cond in enumerate(CONDITIONS):
        ...
        cfg["Q"] = Q_ml * 1e-3  # Convert mL/min to L/min
        
        exp_data = {"t_min": t_data, "P_meas": p_data}
        
        # Initialize walkers around thesis Table 5.5 values
        init_pos = np.array([-5.20, -15.50, 1.16, -4.80]) + 0.05 * np.random.randn(nwalkers, ndim)
        ...
        labels = [r"$\log_{10}(k_P)$", r"$\log_{10}(k_{\mathrm{HA}})$", r"$\Omega^*_{\mathrm{HA}}$", r"$\log_{10}(\sigma_P)$"]
        ...
        fig_c = corner.corner(samples, labels=labels, quantiles=[0.16, 0.50, 0.84], show_titles=True)
        ...
        ax.axhline(cfg["Pin"] * 1e3, color="gray", linestyle=":", label="Inflow Pin (1.0 mM)")
    ...
    for idx, cond in enumerate(CONDITIONS):
        ...
        ax1.hist(s[:, 0], bins=25, density=True, alpha=0.45, color=colors[idx], label=cond["name"])
        ax2.hist(s[:, 1], bins=25, density=True, alpha=0.45, color=colors[idx], label=cond["name"])
        ax3.hist(s[:, 2], bins=25, density=True, alpha=0.45, color=colors[idx], label=cond["name"])
    ...
    ax2.set_xlabel(r"$\log_{10}(k_{\mathrm{HA}})$  $[\mu\mathrm{mol}/(\mathrm{m}^2\cdot\mathrm{min})]$", fontsize=11)
